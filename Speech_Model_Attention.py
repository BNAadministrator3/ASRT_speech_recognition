...
class Speech_Model():
    def __init__(self,batch_size,is_train=True):
        ...
        self.get_mode(is_train)


    def get_mode(self,is_train=True):
        '''
        ...
        fbH1,state,_ = tf.contrib.rnn.stack_bidirectional_dynamic_rnn(lstm_fw_cell,lstm_bw_cess,output_reshape,
        ...
        # self.batch_size = fbH1.get_shape().as_list()[0]

        attn_lstm_cell = tf.nn.rnn_cell.BasicLSTMCell(2*self.lstm_num_hidden)
        # attn_lstm_cell = tf.contrib.rnn.DeviceWrapper(attn_lstm_cell, "/device:GPU:0")
        ...
        if is_train:
            ...
            with tf.variable_scope("output_embedding"):
                output_embedding = tf.get_variable(
                    name="W_embedding",
                    shape=[self.MS_OUTPUT_SIZE, self.embedding_dim],
            ...
            helper = tf.contrib.seq2seq.GreedyEmbeddingHelper(
                ...
                start_tokens=tf.fill([self.batch_size], self.MS_OUTPUT_SIZE),
        ...
        droput_9 = tf.layers.dropout(outputs[0][0], rate=0.4,training=self.is_train)  # 随机丢失层
    ...
    def TestMode(self,data,sess,epoch,txt_obj,batch_size):
        import time
        # 测试数据集
        ...
        for i in range(0,it_count):
            test_data, _ = data.get_data(batch_size, self.MAX_TIME, ran_num=1000+i*batch_size)
        ...
        error_rate = edit_sum / (it_count*batch_size) * 100
